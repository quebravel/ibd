...
umount_partitions() {
  ...
  s | S)
    ...
    (
      ...
      echo rm 4
      echo quit
    ) | parted /dev/${NMSD} &>>$INSTLOG &
    show_progress $!
    #&& dd if=/dev/zero of=/dev/"${NMSD}" bs=1M
    ;;
  n | N)
    echo -e "$CAT - O DISCO NAO SERÁ FORMATADO."
    ;;
  *)
    echo -e "$CER - VOCÊ DIGITOU A LETRA ERRADA."
    umount_partitions
    ;;
  esac
}
...
instalando_kernel() {
  ...
  pacstrap "${MOUNTPOINT}" base base-devel linux linux-firmware &>>$INSTLOG &
  show_progress $!
  sleep 0.2
...
teclado_layout() {
  ...
  b | B)
    ...
    ;;
  u | U)
    arch_chroot "echo 'KEYMAP=us-acentos' > /etc/vconsole.conf"
    ;;
  *)
    echo -e "$CER - VOCE DIGITOU A LETRA ERRADA."
    teclado_layout
    ;;
  esac
}
...
nome_host() {
  ...
  if [[ ! "$HOSTS" =~ ^[a-zA-Z0-9]([a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?$ ]]; then
    echo -e " Hostname inválido. Use apenas letras, números e hífen (sem começar/terminar com hífen)."
    nome_host
    return
  fi
...
criando_usuario_senha() {
  ...
  if ! arch_chroot "passwd $USUARIO"; then
    echo "ERROU A SENHA DO USUARIO"
    criando_usuario_senha
  fi
  sleep 0.2
...
pacotes_extras() {
  ...
  pacstrap "${MOUNTPOINT}" sudo polkit vi openssh --noconfirm --needed &>>$INSTLOG &
  show_progress $!
...
internet_configuracao() {
  ...
  if [[ -n $SEM_FIO_DEV ]]; then
    ...
    arch_chroot "echo -e '[Match] \nName=$SEM_FIO_DEV \n\n[Link] \nRequiredForOnline=routable \n\n[Network] \nDHCP=yes \n\n[DHCPv4] \n\nRouteMetric=600 \n\n[IPv6AcceptRA] \nRouteMetric=600' > /etc/systemd/network/25-wireless.network"
    echo "SEM FIO OK"

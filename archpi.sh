...
if ! rm -rf ~/yaourt
    then
        echo 'Não tem a pasta yaourt, tudo bem'
fi
...
echo 'Instalando X'
if ! sudo pacman -S --noconfirm extra/{xorg-server,xorg-xclipboard,xorg-xset,xorg-xsetroot,ttf-dejavu,xorg-xinit,xorg-drivers}
    then
        echo 'ERRO'
        exit 1
fi
...
then
    echo "Não existe dispositivo Wifi, xmobar não será configurado com wifi."
fi
sleep 1
...
if ! git clone https://aur.archlinux.org/package-query.git ~/package-query 
    then
	echo "Não foi possível clonar package-query"
        exit 1
fi
sleep 1
# 2 yaourt 
if ! cd ~/package-query 
    then
        echo 'ERRO pasta package-query não encontrada'
        exit 1
fi
sleep 1
# 3 yaourt
if ! makepkg -si
    then
        echo 'ERRO não foi possível copilar package-query'
        exit 1
fi
echo -e '\033[01;34mPackage-query copilado!\033[0m'
sleep 1
# 4 yaourt
if ! cd ~/
    then
...
sleep 1
# 5 yaourt
if ! git clone https://aur.archlinux.org/yaourt.git ~/yaourt
    then
        echo 'ERRO não foi possível clonar yaourt'
        exit 1
fi
echo "Yaourt clonado"
sleep 1
...
if ! cd ~/yaourt
    then
        echo 'ERRO pasta yaourt não encontrada'
        exit 1
fi
sleep 1
# 7 yaourt
if ! makepkg -si
    then
        echo 'ERRO não foi possível copilar yaourt'
        exit 1
fi
echo "yaourt copilado"
sleep 1
# 8 yaourt
if ! yaourt -Syu --devel --aur --noconfirm
    then
        echo 'ERRO'
        exit 1
fi
...
if ! sudo pacman -S --noconfirm community/{xmonad,xmonad-contrib,xmonad-utils,stalonetray,rxvt-unicode} extra/lightdm
    then
        echo 'ERRO'
        exit 1
fi
...
# XMOBAR
if ! yaourt -Sy --noconfirm xmobar-alsa
    then
        echo 'ERRO xmobar-alsa não instalado'
        exit 1
fi
echo -e '\033[01;34mXmobar-alsa instalado!\033[0m'
sleep 1
...
sleep 1
# Abilitando lightdm na inicialização
if ! sudo systemctl enable lightdm.service
     then
	 echo "Lightdm não abilitado na inicialização"
	 exit 1
fi
echo "Lightdm abilitado"
sleep 1

# Adicionando autologin ao grupo de usuários
...
if ! xmonad --restart
    then
        echo 'Não foi possível reiniciar o Xmonad ou xmonad ainda não foi iniciado'
fi
...
if ! sudo pacman -S --noconfirm community/awesome extra/lightdm
    then
...
if ! sudo pacman -S --noconfirm extra/{networkmanager,mousepad,gvim,alsa-plugins,moc,pulseaudio,alsa-utils,mplayer,poppler,unrar,unzip,zip,p7zip,w3m,feh,xorg-xbacklight,xorg-xfd,ntfs-3g,xfce4-terminal} community/{ranger,odt2txt,atool,numlockx,rofi,scrot,ctags,xorg-xsetroot,lua} core/{dosfstools,openssh,ncurses}
    then
        echo 'ERRO pacman não conseguiu instalar os programas'
        exit 1
fi
...
# yaourt programas
if ! yaourt -Sy --noconfirm google-chrome ttf-arphic otf-bitstream-charter ttf-droid-min otf-takao wqy-microhei-nightly_build ttf-wqy-zenhei-ibx	otf-fantasque-sans-mono rar
then 
	echo "não foi possível instalar programas com o yaourt"
fi
...
# netbeans
if ! sudo pacman -S --noconfirm community/netbeans extra/jdk8-openjdk
    then
...
if ! git clone https://github.com/sunaku/tamzen-font.git ~/.local/share/fonts/tamzen-font
    then
        echo 'ERRO não foi possível clonar o tamzen-font'
fi
...
echo -e '\033[01;34mFonte ProFont e Monaco instaladas!\033[0m'
sleep 2
if ! xset +fp /usr/share/fonts/75dpi/
then
...
# Instalando fish
if ! sudo pacman -S --noconfirm community/fish
    then
...
# zsh
if ! sudo pacman -S --noconfirm extra/zsh
    then
...
sleep 1
# zsh completions
if ! sudo yaourt -Sy --noconfirm zsh-completions-git
    then
...
if ! sudo pacman --noconfirm extra/xf86-video-intel
    then
        echo 'ERRO'
        exit 1
fi
...
if ! sudo pacman -S --noconfirm extra/xf86-video-amdgpu
    then
...
# Nvidia driver
if ! sudo pacman -S --noconfirm extra/nvidia
    then
